...
def build_pixel_decoder(cfg, input_shape, depth_decoder=False):
    """
    Build a pixel decoder from `cfg.MODEL.ONE_FORMER.PIXEL_DECODER_NAME`.
    """
    name = cfg.MODEL.SEM_SEG_HEAD.PIXEL_DECODER_NAME if not depth_decoder else cfg.MODEL.SEM_SEG_HEAD.DEPTH_DECODER_NAME
...
@SEM_SEG_HEADS_REGISTRY.register()
class DepthTransformerEncoderPixelDecoder(BasePixelDecoder):
    @configurable
    def __init__(
        self,
        input_shape: Dict[str, ShapeSpec],
        *,
        ...
        self.add_module("layer_{}".format(len(self.in_features)), output_conv)
        self.output_convs[0] = output_conv
        self.low_disp_outs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(
                    conv_dim,
                    conv_dim // 2,
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.in_features[::-1]):
            ...
            disps.append(self.low_disp_outs[idx](y))
        return disps
