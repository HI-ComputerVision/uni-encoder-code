...
class MotionDecoder(nn.Module):
    ...
    def __init__(self, num_inp_feat, scales=range(4), num_input_images=2, inp_disp=True, out_dim=4):
        super(MotionDecoder, self).__init__()

        self.org_in_ch = num_input_images * (3 + int(inp_disp))
        self.num_inp_feat = num_inp_feat[::-1] + [self.org_in_ch]
    ...
    def _refine_motion_conv(self, in_dim):
        return nn.Sequential(
            nn.Conv2d(in_dim + self.out_dim, in_dim, kernel_size=3, stride=1, padding=1),
            # nn.ELU(inplace=True),
    ...
    def _upsample_and_concat(self, motion_field, ii):
        out_feat = self.outputs['motion_input']
        out_feat = [out_feat[k] for k in ['full_res_input', 'stem', 'res2', 'res3', 'res4', 'res5']]
        out_feat = out_feat[-1-ii]
    ...
    def _refine_motion_field(self, x):
        ...
        for ii in range(len(self.num_inp_feat)):
            ...
            out[f'x_{ii}_1'] =        conv[:-2](out[f'x_{ii}'])                                                 # (B, num_inp_feat[ii], h, w)
            out[f'x_{ii}_2'] =        conv[-2:](out[f'x_{ii}_1'])                                                # (B, num_inp_feat[ii], h, w)
